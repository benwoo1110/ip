...
public class Parser {
    ...
    public String parseSearchKeyword(ParsedCommand parsedCommand) throws KachowException {
        ...
        if (keyword.isEmpty()) {
            throw new KachowException("Tell me which racer to search for. Use: " + FIND_USAGE);
        }
        return keyword;
    }
    ...
    public int parseTaskNumber(ParsedCommand parsedCommand) throws KachowException {
        ...
        try {
            ...
            throw invalidTaskNumber(action, exception);
        }
        if (taskNumber <= 0) {
            throw invalidTaskNumber(action, null);
        }
        return taskNumber;
    }

    /** Creates consistent guidance for malformed task numbers. */
    private KachowException invalidTaskNumber(Command action, NumberFormatException cause) {
    ...
    private KachowException invalidEventDateTime(String parameter, DateTimeParseException cause) {
